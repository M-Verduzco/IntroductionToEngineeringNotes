% Programa para crear trayectorias en (AMF) Rev Matlab 2020a Versión general. 
% Se deben dar los datos de origen co,fo (x1,y1) y los  datos de destino cd,fd (x2,y2). 
% Si fd=fo es un vector horizontal, si cd=co es un vector vertical. Si es diagonal abs(fd-fo) debe ser igual a abs(cd-co) de otra manera da "trayectoria invalida".
% Grafica los valores de la trayectoria. Emplea: Imagen4AMFFinal.gif
% Solo admite 138 por 138 puntos (pixeles) 

AMF=imread('IMAGENMELROSE.gif');
imshow(AMF) 
co=1; %x1
fo=1; %y1
cd=138; %x2
